...
def check_password():
    ...
    # Use a form for password input to prevent auto-resubmission
...
def show_admin_page():
    ...
    if not responses_df.empty:
        ...
        with tab3:
            ...
            daily_submissions = responses_df.groupby('Date').size().reset_index(name='count')
            
            fig = px.line(
                daily_submissions, 
                x='Date', 
                y='count',
                title='Daily Submission Count'
            )
            st.plotly_chart(fig, use_container_width=True)
    else:
...
def show_skills_form():
    ...
    # Create form for all skill inputs
    with st.form("skills_matrix_form"):
        ...
        if submitted:
            # Update total points before validation
            update_total_points()
            ...
            response_data = {
                ...
                'Submitter Email': st.session_state.current_email,
                **st.session_state.skills
            }
    ...
    **4. Total Points**
    ...
    if not st.session_state.email_submitted:
        with st.form("email_form"):
            ...
            email_submitted = st.form_submit_button("Submit Email")
    ...
    # Show skills form only after email is submitted
